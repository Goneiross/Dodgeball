...
double whichPath(PlayerMap *players, int startID, int targetID,
  ...
  do {
    ...
    for (int childID = 0; childID < children.size(); childID++) {
      ...
      for (int closedChildID = 0; closedChildID < closedList.size(); closedChildID++) {
        if (children[childID]->position == closedList[closedChildID]->position) {
          already = true;
        }
      }
      if (not already) {
        ...
        for (int openChildID = 0; openChildID < openList.size(); openChildID++) {
          if (children[childID]->position == openList[openChildID]->position &&
              children[childID]->beginCost > openList[openChildID]->beginCost) {
            continue;
          }
        }
  ...
  if (nextStepID < 0) {
    if (foundSolution) {
      return angle(
          players->getPlayer(startID)->getX(), players->getPlayer(startID)->getY(),
          players->getPlayer(targetID)->getX(), players->getPlayer(targetID)->getY());
    } else {
      return NO_SOLUTION;
    }
  }
