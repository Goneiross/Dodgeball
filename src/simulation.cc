...
static void parseData(double &ingameMargin, double &parsingMargin, string inputData0, 
											bool &error);
static void parsePlayer(int p, int nbCell, double parsingMargin, double playerRadius, 
												double playerVelocity, string inputData0, string inputData1, 
												string inputData2, string inputData3, bool &error, int mode);
static void parseObstacle(int nbCell, int o, string inputData0, string inputData1, 
													bool &error, int mode);
static void parseBall(int nbCell, int nbPlayer, int nbObstacle, double parsingMargin, 
											int b, double ballRadius, double ballVelocity, string inputData0,
...
bool initialization(string inputFile, int mode) {
  	...
  	int parseType = 0;
		enum parseType {
				cellParsing = 0, playerNumberParsing, playerParsing, obstacleNumberParsing, 
				obstacleParsing, ballNumberParsing, ballParsing, 
		};
    		...
    		if (inputData[0] == "#") {
      			do {
						...
						case playerParsing:
      					...
      					parsePlayer(p, nbCell, parsingMargin, playerRadius, playerVelocity, 
						...
						case ballParsing:
      					...
      					parseBall(nbCell, nbPlayer, nbObstacle, parsingMargin, b, 
               						ballRadius, ballVelocity, inputData[0], inputData[1], 
													inputData[2], error, mode);
      					b++;
    		...
    		if (isOut(balls->getBall(b)->getX(), balls->getBall(b)->getY())){
				...
				for (int p = 0; p < playerNb; p++){
						...
						if (collision){
								cout << "REMOVE" << endl;
								balls->removeBall(b);
								players->getPlayer(p)->setTimeTouched(players->getPlayer(p)
											->getTimeTouched() - 1);
						}
				}
  	}
		...
		for (int i = leftL; i <= rightL; i++){
    		for (int j = leftC; j <= rightC; j++){
      			if(mainMap->isObstacle(lgnPos + i, colPos + j)){
        				toCheck.push_back(mainMap->whichObstacle(lgnPos + i, 
																	colPos + j));
      			} 
    		}
  	}
...
static void collisionCheckBB( int b, int c, double delta, bool &error, int mode) {
  	double d = distance(balls->getBall(c)->getHitbox(), 
								balls->getBall(b)->getHitbox());
  	if (d < (balls->getBall(c)->getRadius() + balls->getBall(b)->getRadius() 
				+ delta)) {
