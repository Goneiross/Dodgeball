  ...
  while (flux >> inputData[0]) {
    if (error) {
      ...
      case playerParsing:
        ...
        parsePlayer(p, parsingMargin, playerRadius, playerVelocity, inputData[0],
      ...
      case ballParsing:
        ...
        parseBall(nbPlayer, nbObstacle, parsingMargin, b, ballRadius, ballVelocity,
                  inputData[0], inputData[1], inputData[2], error, mode);
        b++;
...
static void collisionCheckPB(int p, int b, double delta, bool &error, int mode) {
  ...
  if (d <
      (players->getPlayer(p)->getRadius() + balls->getBall(b)->getRadius() + delta)) {
...
static void collisionCheckBO(int b, int o, double delta, bool &error, int mode) {
  double d =
      distance(obstacles->getObstacle(o)->getHitbox(), balls->getBall(b)->getHitbox());
...
static void parseBall(int nbPlayer, int nbObstacle, double parsingMargin, int b,
  ...
  if (isOut(stod(inputData0), stod(inputData1))) {
    ...
    balls->addBall(stod(inputData0), stod(inputData1), stod(inputData2), ballRadius,
                   ballVelocity, b);
    vector<int> toCheck;
