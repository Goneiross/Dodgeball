...
static void largeCollisionCheck(PlayerMap* players, Map* map, int p,
                                vector<int>& toCheck);
static void largeCollisionCheck(BallMap* balls, Map* map, int b, vector<int>& toCheck);
static void largeCollisionCheck(BallMap* balls, int b, vector<int>& toCheck, int lMax);
static void largeCollisionCheck(PlayerMap* players, int b, vector<int>& toCheck,
                                int lMax);
static void collisionCheck(PlayerMap* players, int p, int pmax, double delta,
                           bool& error);
static void collisionCheck(BallMap* balls, int b, int bmax, double delta, bool& error);
static void collisionCheck(PlayerMap* players, BallMap* balls, int p, int b,
                           double delta, bool& error);
static void collisionCheck(PlayerMap* players, Map* map, int p, int o, double delta,
                           bool& error);
static void collisionCheck(BallMap* balls, Map* map, int b, int o, double delta,
                           bool& error);
...
void simulation(std::string inputFile, int mode) {
    ...
    if (inputFile != "") {
        success = initialization(inputFile, players, mainMap, balls);
    }
...
bool initialization(string inputFile, PlayerMap*& players, Map*& mainMap,
                    BallMap*& balls) {
    bool error = false;
    ...
    while (flux >> inputData[0]) {
        if (inputData[0] == "#") {
            ...
            parseData(mainMap, players, balls, ingameMargin, parsingMargin,
                      inputData[0], error);
            ...
            parseData(players, p, nbCell, parsingMargin, playerRadius, playerVelocity,
                      inputData[0], inputData[1], inputData[2], inputData[3], error);
            p++;
            if (p == nbPlayer) {
                parseType++;
            }
            ...
            flux >> inputData[1];
            parseData(mainMap, nbCell, o, inputData[0], inputData[1], error);
            o++;
            if (o == nbObstacle) {
            ...
            parseData(balls, players, mainMap, nbCell, nbPlayer, nbObstacle,
                      parsingMargin, b, ballRadius, ballVelocity, inputData[0],
                      inputData[1], inputData[2], error);
            b++;
            if (b == nbBall) {
                for (int i = 0; i < nbBall; i++) {
                    ...
                    for (auto o : toCheck) {
                        collisionCheck(balls, mainMap, i, o, parsingMargin, error);
                    }
                }
                parseType++;
            }
...
void save(string filename, int nbCell, PlayerMap* players, Map* mainMap,
          BallMap* balls) {
...
static void largeCollisionCheck(PlayerMap* players, Map* map, int p,
    ...
    if (lPosition == map->getCNb() - 1) {
        rightL = 0;
    }
    if (cPosition == map->getLNb() - 1) {
        rightC = 0;
    }
...
static void largeCollisionCheck(BallMap* balls, int b, vector<int>& toCheck,
                                int lMax) {
...
static void largeCollisionCheck(PlayerMap* players, int p, vector<int>& toCheck,
                                int lMax) {
...
static void collisionCheck(BallMap* balls, int b, int c, double delta, bool& error) {
    double d =
...
static void collisionCheck(PlayerMap* players, Map* map, int p, int o, double delta,
                           bool& error) {
    double d =
        distance(map->getObstacle(o)->getHitbox(), players->getPlayer(p)->getHitbox());
...
static void collisionCheck(BallMap* balls, Map* map, int b, int o, double delta,
                           bool& error) {
    double d =
        distance(map->getObstacle(o)->getHitbox(), balls->getBall(b)->getHitbox());
...
static void parseData(PlayerMap*& players, int p, int nbCell, double parsingMargin,
    ...
    if (((abs(stod(inputData0)) > DIM_MAX) || (abs(stod(inputData1) > DIM_MAX)))) {
        ...
        for (auto c : toCheck) {
            collisionCheck(players, p, c, parsingMargin, error);
        }
    }
}
...
static void parseData(BallMap*& balls, PlayerMap*& players, Map*& mainMap, int nbCell,
    ...
    if (isOut(stod(inputData0), stod(inputData1))) {
        ...
        for (auto c : toCheck) {
            collisionCheck(balls, b, c, parsingMargin, error);
        }
        for (int i = 0; i < nbPlayer; i++) {
            collisionCheck(players, balls, i, b, parsingMargin, error);
        }
    }
}