...
void pathfinding(PlayerMap* players, ObstacleMap* obstacles){
    ...
    for (int p = 0; p < players->getNb(); p++){
        ...
        pathAngles[p] = whichPath(players->getPlayer(p), players->getPlayer(targets[p]), infinityInit, infinityDist, obstacles, firstInStep); 
        cout << "Player " << players->getPlayer(p)->getID() << " angle: " << pathAngles[p]*180/M_PI << " degrés" << endl;
	    cout << endl;
    }
}
