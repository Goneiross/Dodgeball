...
void complexPath(Player* start, Player* target, int infinityInit, int infinityDist, bool &firstInStep);
...
vector<int> targetting(PlayerMap* players, int infinityInit, int infinityDist){
	...
	for (int i=0; i < players->getNb(); i++) {
		for (int j=0; j < i; j++) {
			...
			dBP[j][i] = dBP[i][j];
			cout << "start ID: " << i << "(" << players->getPlayer(i)->getL() << "," << players->getPlayer(i)->getC() << ") target ID: " << j << "(" << players->getPlayer(j)->getL() << "," << players->getPlayer(j)->getC() << ") distance:" << distance(players->getPlayer(i)->getL(), players->getPlayer(i)->getC(), players->getPlayer(j)->getL(), players->getPlayer(j)->getC()) << endl;
...
double complexPath(Player* start, Player* target, int infinityInit, int infinityDist, ObstacleMap* obstacles,bool &firstInStep){
